...
mod4 <- lm(formula = value ~ as.numeric(YEAR), data = filter(df, COUNTY == "001", Grade == "-1"), na.action = na.omit)
...
###This creates 2 year averages of attrition rate:
modTPMa_rate <- lm(formula = attrition_rate ~ as.factor(CATEGORY)*region_subject, 
              data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 3) | YEAR == current_year)), 
              na.action = na.omit)

###4 year averages for ST-ratio:
modTPM_ST_Ratio <- lm(formula = ST_ratio ~ as.factor(CATEGORY)*region_subject, 
                   data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 4))), 
                   na.action = na.omit)

###2 year averages for Not Appropriately Certified:
modTPM_NAC_rate <- lm(formula = not_fully_cert_rate ~ as.factor(CATEGORY)*region_subject, 
                   data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 2))), 
                   na.action = na.omit)

###2-year average Enrollment
modTPM_enrollment_2 <- lm(formula = enrollment_count ~ as.factor(CATEGORY)*region_subject, 
                      data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 2))), 
                      na.action = na.omit)

modTPM_enrollment_4 <- lm(formula = enrollment_count ~ as.factor(CATEGORY)*region_subject, 
                          data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 4))), 
                          na.action = na.omit)

modTPM_enrollment_trend <- lm(formula = enrollment_count ~ as.factor(CATEGORY)*region_subject*YEAR, 
                          data = filter(TPMsubtransformed, !n %in% c(Inf, NaN, NA), !(YEAR <= (current_year - 4))), 
                          na.action = na.omit)
...
TPMsubtransformed$ST_ratio <- ifelse(TPMsubtransformed$YEAR > current_year, NA, TPMsubtransformed$ST_ratio)
#Make predictions
TPMsubtransformed <- filter(TPMsubtransformed, !region %in% c("xxother", "null"))

newdata <- TPMsubtransformed[TPMsubtransformed$YEAR >= current_year,]
newdata2 <- TPMsubtransformed[TPMsubtransformed$YEAR > current_year,]

newdata$preds <- predict(object = modTPMa_rate, newdata = newdata)
newdata2$predST <- predict(object = modTPM_ST_Ratio, newdata = newdata2)
newdata2$predNAC <- predict(object = modTPM_NAC_rate, newdata = newdata2)
newdata2$predenroll2 <- predict(object = modTPM_enrollment_2, newdata = newdata2)
newdata2$predenroll4 <- predict(object = modTPM_enrollment_4, newdata = newdata2)
newdata2$predenrollTRD <- predict(object = modTPM_enrollment_trend, newdata = newdata2)

TPMsubtransformed <- merge(TPMsubtransformed, newdata, all.x = TRUE)  
TPMsubtransformed <- merge(TPMsubtransformed, newdata2, all.x = TRUE)  

TPMsubtransformed$Attrition <- ifelse(TPMsubtransformed$YEAR >= c(current_year), TPMsubtransformed$preds, TPMsubtransformed$attrition_rate)
TPMsubtransformed$ST_ratio <- ifelse(TPMsubtransformed$YEAR > c((current_year)), TPMsubtransformed$predST, TPMsubtransformed$ST_ratio)
TPMsubtransformed$not_fully_cert_rate <- ifelse(TPMsubtransformed$YEAR > c((current_year)), TPMsubtransformed$predNAC, TPMsubtransformed$not_fully_cert_rate)
...
TPMsubtransformed$Region <- stringr::str_to_title(string = TPMsubtransformed$Region)
