...
server <- function(input, output) {
    ...
    pred <- reactive({
        ...
        model_fun <- function(var, name, type, subset = FALSE, round = FALSE, ponly = FALSE, currentyear = current_year){
            ...
            if(subset == TRUE){
                datsub <- subset(datsub, select = c(Region, Year, p,trendyear))
            } 
    ...
    pred_subj <- reactive({
        ...
        if(input$Variable_subj %in% c("Attrition","Not Fully Certified Rate")){
            datsub <- data.frame(x = dat[,input$Variable_subj], `Subject Area` = dat$`Subject Area`, Year = dat$Year)
            names(datsub)[names(datsub) == "Subject.Area"] <- "Subject Area"
            lm1 <- lm(x ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-1):current_year),])
            datsub$p <- predict(object = lm1, newdata = datsub)
            ...
            datsub <- data.frame(x = dat[,input$Variable_subj], `Subject Area` = dat$`Subject Area`, Year = dat$Year)
            names(datsub)[names(datsub) == "Subject.Area"] <- "Subject Area"
            lm1 <- lm(x ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-3):current_year),])
            datsub$p <- predict(object = lm1, newdata = datsub)
            datsub$p <- round(ifelse(datsub$Year <= current_year, datsub$x, datsub$p),4)
            datsub <- subset(datsub, select = c(`Subject Area`, Year, p))
            names(datsub)[names(datsub) == "p"] <- input$Variable_subj
            ...
            datsub <- data.frame(x = dat$enrollment_count, st = dat$`Student/Teacher Ratio`, teach_count = dat$`Number of Teachers`, `Subject Area` = dat$`Subject Area`, Year = dat$Year)
            names(datsub)[names(datsub) == "Subject.Area"] <- "Subject Area"
            lm1 <- lm(st ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-3):current_year),])
            ...
            names(datsub)[names(datsub) == "Subject.Area"] <- "Subject Area"
            #Predict Number of teachers
            lm1 <- lm(st ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-2):current_year + 1),])
            datsub$p_st <- predict(object = lm1, newdata = datsub)
            datsub$p_st <- ifelse(datsub$Year <= current_year + 1, datsub$st, datsub$p_st)
            datsub$p_nt <- datsub$p_st * datsub$x
            
            #Predict number of leavers
            lm2 <- lm(ar ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-3):current_year),])
            ...
            #Predict number of leavers
            lm2 <- lm(ar ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-4):current_year-1),])
            datsub$p_ar <- predict(object = lm2, newdata = datsub)
            ...
            #Predict certification rate
            lm2 <- lm(nfcr ~ `Subject Area`, data = datsub[datsub$Year %in% c((current_year-1):current_year),])
