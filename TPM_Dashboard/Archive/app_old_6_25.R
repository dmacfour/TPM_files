...
ui <- navbarPage("Teacher Predictor Model",
    ...
    # Application title
        ...
        sidebarLayout(
            ...
            sliderInput("range", "Year Range", min = 2010, max = 2025, value = c(2013,current_year+4), sep = ""),
...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        ggplot(data = NULL) + 
            ...
            stat_summary(aes(x = datsub[datsub$Year >= current_year,]$Year, y = datsub[datsub$Year >= current_year,input$Variable], group = datsub[datsub$Year >= current_year,"Region"], color = datsub[datsub$Year >= current_year,"Region"]), linetype = "longdash",fun.y = identity, geom = "line", size  = .9) +
            stat_summary(aes(x = datsub[datsub$Year >= current_year,]$Year, y = datsub[datsub$Year >= current_year,input$Variable], group = datsub[datsub$Year >= current_year,"Region"], color = datsub[datsub$Year >= current_year,"Region"]), fun.y = identity, geom = "point", size  = 2) +
            stat_summary(aes(x = dat[dat$Year <= current_year,]$Year, y = dat[dat$Year <= current_year,input$Variable]), fun.y = mean, geom = "line", size  = .9) + 
            stat_summary(aes(x = dat[dat$Year <= current_year,]$Year, y = dat[dat$Year <= current_year,input$Variable]), fun.y = mean, geom = "point", size  = 2) +
            stat_summary(aes(x = datsub[datsub$Year >= current_year,]$Year, y = datsub[datsub$Year >= current_year,input$Variable]), linetype = "longdash",fun.y = mean, geom = "line", size  = .9) +
            stat_summary(aes(x = datsub[datsub$Year >= current_year,]$Year, y = datsub[datsub$Year >= current_year,input$Variable]), fun.y = mean, geom = "point", size  = 2) +
